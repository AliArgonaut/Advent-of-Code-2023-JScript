...
function partTwoSolve(){
    ...
    let removedSharedLetterList = newArr.map(item => {return item.replace(/oneight/g, '18')
    .replace(/twone/g, '21')
    ...
    let fullConversionList = removedSharedLetterList.map(item => {return item.replace(/one/g, '1')
    .replace(/two/g, '2')
    .replace(/three/g, '3')
    .replace(/four/g, '4')
    .replace(/five/g, '5')
    ...
    FinalResult2 = p2Output.map(str => parseInt(str, 10)).reduce((p, c) => {return p + c}, 0)
